...
def SecAlgo(level, seed):

    temp_key = bytearray(seed)

    output_key = bytearray(seed)
    xorkey = bytearray(b'\x12\x6F\x03\x45')
    #print(temp_key)
    #print(len(temp_key))
    #print(xorkey)
    #print(len(xorkey))
    ...
    output_key[0] = (temp_key[3] & 0x0F) | (temp_key[2] & 0xF0)
    output_key[1] = ((temp_key[2] & 0x1F) << 3) | ((temp_key[1] & 0xF8) >> 3)
    output_key[2] = ((temp_key[1] & 0xFC) >> 2) | (temp_key[0] & 0xC0)
    output_key[3] = ((temp_key[0] & 0x0F) << 4) | (temp_key[3] & 0x0F)
        
    return bytes(output_key)

seed = bytearray(b'\x40\x74\x16\x23')
print("output_key" , binascii.hexlify(SecAlgo(1, seed)))
print(len(SecAlgo(1, seed)))

temp_key = 0x12345678

output_key = ((((temp_key >> 4) ^ temp_key) << 3) ^ temp_key) & 0xFFFFFFFF
print(hex(output_key))
